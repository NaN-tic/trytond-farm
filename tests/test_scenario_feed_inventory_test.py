...
from trytond.modules.farm.tests.tools import (create_feed_product,
                                              create_specie, create_users)
...
class Test(unittest.TestCase):
    ...
    def test(self):
        ...
        # Put 2000 Kg of first Lot of Feed into the silo 10 days before, and 1500 Kg of        # second Lot of Feed 3 days before
        ...
        provisioning_move1.planned_date = now.date() - datetime.timedelta(
            days=8)
        provisioning_move1.effective_date = now.date() - datetime.timedelta(
            days=8)
        ...
        provisioning_move2.planned_date = now.date() - datetime.timedelta(
            days=3)
        provisioning_move2.effective_date = now.date() - datetime.timedelta(
            days=3)
        ...
        # Create initial (real) feed inventory for silo S1 and silo's locations to fed at        # 8 days before
        ...
        self.assertEqual(feed_inventory0.state, 'draft')
        self.assertEqual(
            set([l.id for l in feed_inventory0.dest_locations
                 ]) == set([location1.id, location2.id, location3.id]), True)

        # Confirm initial feed inventory. As it is the initial, it doesn't have any line        # nor feed event
        feed_inventory0.click('confirm')
        self.assertEqual(feed_inventory0.state, 'validated')
        self.assertEqual(feed_inventory0.feed_events, [])

        # Create first privisional feed inventory for silo S1 and silo's locations to fed        # with 1000.00 Kg at 5 days before
        ...
        self.assertEqual(feed_provisional_inventory1.state, 'draft')

        # Confirm first provisional feed inventory and check it has an stock inventory in        # state 'Done' and the median of Consumed Quantity per Animal/Day is        # approximately 50 Kg
        feed_provisional_inventory1.click('confirm')
        self.assertEqual(feed_provisional_inventory1.state, 'validated')
        self.assertEqual(
            feed_provisional_inventory1.feed_events[0].feed_quantity_animal_day,
            Decimal('52.6316'))
        self.assertEqual(feed_provisional_inventory1.inventory.state, 'done')

        # Create second privisional feed inventory for silo S1 and silo's locations to        # fed with 1100.00 Kg at 2 days before
        ...
        self.assertEqual(feed_provisional_inventory2.state, 'draft')

        # Confirm second provisional feed inventory and check it has an stock inventory        # state 'Done' and the median of Consumed Quantity per Animal/Day is        # approximately 50 Kg
        feed_provisional_inventory2.click('confirm')
        self.assertEqual(feed_provisional_inventory2.state, 'validated')
        self.assertEqual(
            feed_provisional_inventory2.feed_events[0].feed_quantity_animal_day,
            Decimal('58.3333'))
        self.assertEqual(feed_provisional_inventory2.inventory.state, 'done')

        # Create (real) feed inventory for silo S1 and silo's locations to fed with        # 200.00 Kg at today
        ...
        self.assertEqual(feed_inventory1.state, 'draft')

        # Confirm feed inventory. Check the current stock of Silo is 200.00 Kg and the        # current lot is the second Feed Lot
        feed_inventory1.click('confirm')
        self.assertEqual(feed_inventory1.state, 'validated')
        silo1.reload()
        self.assertEqual(silo1.current_lot, feed_lot2)
        config._context['locations'] = [silo1.id]
        self.assertEqual(
            (Decimal(feed_lot2.quantity).quantize(Decimal('0.01')) -
             Decimal('200.00')) < Decimal('0.01'), True)

        # Check provisional inventories doesn't have stock inventory related (it has been        # removed)
        feed_provisional_inventory2.reload()
        self.assertEqual(feed_provisional_inventory2.inventory, None)
