...
class Test(unittest.TestCase):
    ...
    def test(self):
        ...
        dose_bom = Bom(
            ...
            specie=specie,
            inputs=[
                BomInput(product=blister_product, unit=unit, quantity=1),
                BomInput(product=semen_product, unit=cm3, quantity=100.00),
            ],
            outputs=[
                BomOutput(product=dose_product, unit=unit, quantity=1.00),
            ],
        )
        dose_bom.save()
        dose_bom.reload()
