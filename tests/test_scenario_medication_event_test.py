...
class Test(unittest.TestCase):
    ...
    def test(self):
        ...
        self.assertEqual(medication_individual.state, 'validated')
        ...
        medication_animal_group.start_date = now.date() - datetime.timedelta(
            days=1)
        medication_animal_group.medication_end_date = now.date(
        ...
        self.assertEqual(medication_animal_group.state, 'validated')
        animal_group.reload()
        config._context['locations'] = [lab1.id]
        medication_lot = Lot(medication_lot.id)
        self.assertEqual(medication_lot.quantity, 26.0)
